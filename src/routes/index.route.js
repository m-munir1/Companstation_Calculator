	...
	try {
		...
		if (Number(user.otp) === Number(otp)) {
			...
			try {
				...
				return res.status(200).send({ success: true, message: isEmailSent.error ? 'Order Confirmed, but failed to send email' : 'An email is sent to your id' });
			} catch (error) {
				console.log({ error });
